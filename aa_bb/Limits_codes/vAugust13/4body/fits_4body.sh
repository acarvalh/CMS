...
#  for (( j = 0 ; j < 1 ; j++ )); do # for each mass 
	...
	# the signal file
	sed -i -r -e "s/WP[0-9]/WP$i/g" R2GGBBFitter_mtot.cc
        sed -i -r -e "s/m[0-9]+.root/m${radion[$j]}.root/g" R2GGBBFitter_mtot.cc
        sed -i -r -e "s/m[0-9]+_8TeV_nm_m[0-9]+.root/m${radion[$j]}_8TeV_nm_m${radion[$j]}.root/g" R2GGBBFitter_mtot.cc
#	sed -i -r -e "s/[0-9]+\_minimal.root/${radion[$j]}\_minimal.root/g" R2GGBBFitter_mtot.cc
#	sed -i -r -e "s/[0-9]+\_regression/${radion[$j]}\_regression/g" R2GGBBFitter_mtot.cc
...
#	echo did with lnu 
	#
        combine hgg.mH${radion[$j]}.0_8TeVonecat.txt -M Asymptotic >> higgsCombineTest.Asymptotic.mH125.mR${radion[$j]}onecat.txt
	mv higgsCombineTest.Asymptotic.mH${radion[$j]}.root higgsCombineTest.Asymptotic.mR${radion[$j]}_onecat.root
	echo did with rep 2btag
	#
	combine hgg.mH${radion[$j]}.0_8TeVrep.txt -M Asymptotic -S 0 >> higgsCombineTest.Asymptotic.mH125.mR${radion[$j]}_nosyst.txt
	mv higgsCombineTest.Asymptotic.mH${radion[$j]}.root higgsCombineTest.Asymptotic.mR${radion[$j]}_nosyst.root
	echo with rep no syst
	combine hgg.mH${radion[$j]}.0_8TeVrep.txt -M Asymptotic -m ${radion[$j]} >> higgsCombineTest.Asymptotic.mH125.mR${radion[$j]}_reparametrization.txt
	mv higgsCombineTest.Asymptotic.mH${radion[$j]}.root higgsCombineTest.Asymptotic.mH125.mR${radion[$j]}_rep.root
	echo did with rep 
	cd ../..
  done # mass
